...
const normalizeType = function(property, k) {
    ...
    } else if (isSupportedType(property.constructor)) {
        typeDeclaration.type = property.constructor;
    } else {
        console.warn('Unsupported type or bad variable. ' +
            'Remember, non-persisted objects must start with an underscore (_). Field:' + k + ' Got: ' + property);
...
class BaseDocument {
    ...
    generateSchema() {
        ...
        _.keys(this).forEach(function(k) {
            ...
            that._schema[k] = normalizeType(that[k], k);
